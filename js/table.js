...
import { n, fmt, pct, cleanName, getStatus } from './utils.js?v=33';
import { renderSunburst, updateLegend } from './sunburst.js?v=33';
...
export function renderSampleList(samples) {
    ...
    container.querySelectorAll('.sample-item').forEach(item => {
        item.addEventListener('click', async () => {
            const { selectSample } = await import('./actions.js?v=33');
...
export function renderTable() {
    ...
    tbody.querySelectorAll('tr').forEach(row => {
        row.addEventListener('click', async (e) => {
            const { selectReference, toggleCompare } = await import('./actions.js?v=33');
    ...
    menu.querySelectorAll('.context-menu-item').forEach(item => {
        item.addEventListener('click', async () => {
            ...
            if (action === 'track-species' || action === 'track-genus' || action === 'track-family') {
                const { trackTaxonAcrossSamples } = await import('./compare.js?v=33');
                const level = action.replace('track-', '');
                trackTaxonAcrossSamples(taxon, level);
            } else if (action === 'add-compare') {
                const { addToBasket } = await import('./compare.js?v=33');
