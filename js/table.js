// ========== TABLE RENDERING ==========
import { state, domainColors } from './state.js?v=31';
import { n, fmt, pct, cleanName, getStatus } from './utils.js?v=31';
import { renderSunburst, updateLegend } from './sunburst.js?v=31';
...
export function renderTable() {
    ...
    tbody.querySelectorAll('tr').forEach(row => {
        row.addEventListener('click', async (e) => {
            const { selectReference, toggleCompare } = await import('./actions.js?v=31');
    ...
    menu.querySelectorAll('.context-menu-item').forEach(item => {
        item.addEventListener('click', async () => {
            ...
            if (action === 'track-species' || action === 'track-genus' || action === 'track-family') {
                const { trackTaxonAcrossSamples } = await import('./compare.js?v=31');
                const level = action.replace('track-', '');
                trackTaxonAcrossSamples(taxon, level);
            } else if (action === 'add-compare') {
                const { addToBasket } = await import('./compare.js?v=31');
                ...
                const { selectReference } = await import('./actions.js?v=31');
